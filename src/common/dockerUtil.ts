...
const AVAILABLE_COMMANDS = {
  ...
  STOP_OPENDEX_DOCKER: "stop_opendex_docker",
  SETUP_OPENDEX_DOCKER: "setup_opendex_docker",
};
...
const startOpendexDocker$ = (): Observable<boolean> => {
  return execCommand$(AVAILABLE_COMMANDS.SETUP_OPENDEX_DOCKER).pipe(
    map((output) => {
      logInfo("output for start opendex-docker", output);
      return true;
    })
  );
};

const isOpendexDockerEnvCreated = (network: Network): boolean => {
  return (window as any).electron.opendexDockerEnvExists(network);
};
